...
data = read.csv2("./Data-Set/Dataset-FINALFORPRESENTATION.csv",stringsAsFactors = FALSE)
class(data$Date)
data$Date <- as.Date(data$Date, format = "%d.%m.%Y")
class(data$Date)
names(data)[4:7] = c("A.MCAP", "BVE.MCAP", "D.MCAP","NI")

data0 = data[!(data$Company==3 | data$Company==8),]
...
stargazer(repre2008, rePost2008, type="text", 
          ...
          out="./Quantlet 5/Marcus DATA/Regression-2008.txt")
...
#Random Effect with Dummy => does not work with =BVE

DumyReg = plm(Stock ~ A.MCAP+ NI + BVE.MCAP
              + D.MCAP + Oil + Gas + Market + EURUSD + DumP
...
out="./Quantlet 5/Marcus DATA/Regression-Dummy2008.txt")
...
stargazer(Quarter,type="text",
          ...
          covariate.labels=c("Assets/market cap.","Net income","BVE/market cap.",
                             "Debt/equity","Oil price","Gas price", "DJI premium","EUR/Dollar"), 
          out="./Quantlet 5/Marcus DATA/QuarterDummy.txt")
...
OilC = data0
OtherC = pdata.frame(subset(data,data$Company == 3 | data$Company == 8),
                     index = c("Company", "Date"), 
                     drop.index = F, 
                     row.names = T) 
#Random Effect#
...
#OtherC => does not work with Debt....
...
stargazer(reOilC, reOtherC, type="text", 
          ...
          covariate.labels=c("Assets/market cap.","Net income","BVE/market cap.",
                             "Debt/equity","Oil price","Gas price", "DJI premium","EUR/Dollar"), 
          out="./Quantlet 5/Marcus DATA/Regression.Oil-Other.txt")

############### Regression with a Dummy ################

DumFirmT = ifelse(data$Company==3 | data$Company==8 ,1,0)
length(DumFirmT)
...
stargazer(TypeFirm,type="text",
          ...
          out="./Quantlet 5/Marcus DATA/Regression.DummyOil-Other.txt")
