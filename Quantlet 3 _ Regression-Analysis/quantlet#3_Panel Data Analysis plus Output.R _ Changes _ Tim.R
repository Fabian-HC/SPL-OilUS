...
names(data)[4:6] = c("A.MCAP", "BVE.MCAP", "D.MCAP")
...
stargazer(fe,title="Oneway (individual) effect Within Model",dep.var.labels=c("Stock return"),
          covariate.labels=c(colnames(data[,4:10])), out="femodel.LATEX")

# "Assets over market cap.","Net income","Book value equity over market cap.",
...
stargazer(fe,title="Oneway (individual) effect Random Effect Model (Swamy-Arora's transformation)",
          dep.var.labels=c("Stock return"),covariate.labels=c(colnames(data[,4:10])), 
          out="remodel.LATEX")
...
# We might have to studentize, if the sample is small!
...
stargazer(arellano,title="Arellano model",dep.var.labels=c("Stock return"),
          covariate.labels=c(colnames(data[,4:10])) , out="arellanomodel.LATEX")
...
stargazer(DriscollandKray,title="cross-sectional robust",dep.var.labels=c("Stock return"),
          covariate.labels=c(colnames(data[,4:10])), out="DriscollandKray.LATEX")


# GLS
repggls = pggls(Stock ~ A.MCAP +NI + BVE.MCAP + D.MCAP +Market+ Oil+Gas, 
                model = "pooling", data=data)
SumGLS = summary(repggls)
...
#pooltest
Pool = pooltest(Stock~A.MCAP +NI + BVE.MCAP + D.MCAP +Market+ Oil+Gas,data=data,
        model="within")

#two-ways test

pFtest(Stock~A.MCAP +NI + BVE.MCAP + D.MCAP +Market+ Oil+Gas,data=data,
       effect="twoways")

#unobserved effects test

pwtest(Stock~A.MCAP +NI + BVE.MCAP + D.MCAP +Market+ Oil+Gas,data=data)
