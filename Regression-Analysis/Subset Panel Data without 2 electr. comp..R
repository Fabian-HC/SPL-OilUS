...
#fixed effects model
fe2 <- plm(Stock ~ Assets.to.Market.Cap +Net.Income + BV.Equity.to.Market.Cap + Debt.to.Equity+Oil+Gas+Market+EURUSD, 
       model = "within", data=Subsetdata)
summary(fe2)

#random effects model
re2 <- plm(Stock ~ Assets.to.Market.Cap +Net.Income + BV.Equity.to.Market.Cap + Debt.to.Equity+Oil+Gas+Market+EURUSD, 
       model = "random", data=Subsetdata)
summary(re2)
